...
namespace MonoServices.Core
{
    public abstract class MonoService : CoroutineMonoService
    {
        ...
        protected void InvokeCommand(int methodNumb, object passedObj = null)
        {
            ...
            {
                var emptyCommandsList = receiver.MainMonoService.MonoServiceParams.MonoServiceCommands.Length == 0;

                if (receiver.MainMonoService == null || !receiver.MainMonoService.isActiveAndEnabled || emptyCommandsList)
                    continue;
